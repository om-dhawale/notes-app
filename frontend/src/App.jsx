...
const API_URL = 'https://notes-app-i8mn.onrender.com';
        ...
        <div className="max-w-4xl mx-auto">
          <header className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-bold text-slate-800">
              {selectedGroupId ? 'Group Notes' : 'All My Notes'}
            </h1>
            <div className="relative">
              <Search className="absolute left-3 top-2.5 text-slate-400" size={18} />
              <input 
                className="p-2 pl-10 rounded-full border border-slate-200 outline-none w-64 focus:ring-2 focus:ring-indigo-500"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredNotes.map(note => (
              <div key={note.id} onClick={() => setEditNote(note)} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-indigo-400 transition cursor-pointer group relative flex flex-col min-h-[160px]">
                ...
                <div className="mb-3">
                  <h3 className="font-bold text-lg text-slate-800 pr-10 line-clamp-1">{note.title}</h3>
